...
export const ESP32_CONFIG = {
  SERVICE_UUID: '12345678-1234-1234-1234-123456789abd',
  CHARACTERISTICS: {
...
class BluetoothManager {
  ...
  async writeCharacteristic(characteristicName, data, retries = 2) {
    ...
    const chunkSize = 100 // Conservative chunk size for BLE
      ...
      try {
        if (encodedData.length <= chunkSize) {
          ...
          for (let i = 0; i < encodedData.length; i += chunkSize) {
            ...
            // Small delay between chunks to prevent overwhelming the ESP32
